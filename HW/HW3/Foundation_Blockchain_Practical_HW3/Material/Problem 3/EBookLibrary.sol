...
contract EBookLibrary is CustomERC20{
    ...
    onlyAdmin
    public{
	books[bookId].isValid = true;
    books[bookId].bookGenre = BookGenre(bookGenre);
    books[bookId].accessFee = accessFee;
    }
    ...
    public{
    if (rentedBooks[bookId].available==false)
        {
            if (accoutTypes[msg.sender]==AccountType.premium && 
            accoutTypes[rentedBooks[bookId].renter]==AccountType.regular)
            {
                _transfer(msg.sender, admin, books[bookId].accessFee);
                rentedBooks[bookId].renter = msg.sender;
                rentedBooks[bookId].startTime = now;
                rentedBooks[bookId].available = false;
            }
            else if (now-rentedBooks[bookId].startTime>accessDuration)
            {
                _transfer(msg.sender, admin, books[bookId].accessFee);
                rentedBooks[bookId].renter = msg.sender;
                rentedBooks[bookId].startTime = now;
        ...
        {
            require(rentedBooks[bookId].available==true);
            _transfer(msg.sender, admin, books[bookId].accessFee);
            rentedBooks[bookId].renter = msg.sender;
            rentedBooks[bookId].startTime = now;
            rentedBooks[bookId].available = false;
